...
os.chdir(current_path)  # not using sys.path[0] because pyinstaller
...
class RegFile:
    ...
    def run(self):
        ...
        if self.powerrun:
            os.system(
                f'tweaks\\PowerRun.exe regedit.exe {" /s" if self.silent else ""} "tweaks/{self.file}.reg"'
            )
        else:
            os.system(
                f'regedit.exe {" /s" if self.silent else ""} "tweaks\\{self.file}.reg"'
            )
...
class CMDTweak(RegFile):
    ...
    def run(self):
        print(f"Applying: {self.name} tweak!")
        os.system(f"tweaks\\{self.file}.bat")
...
def CheckHPET():
    print(
        'We opened the app, check the "Current resolution". If it is 1.000 or ~15.000, it\'s okay.'
    )
...
def MsiModeTools():
    print(
        "We opened the Msi Mode Tool application. Find all your video cards, enable MSI, and select High instead of undefined. Then, on the top right, click on apply."
    )
...
def removeUWP(uwp: str, revert=False):
    ...
    if not revert:
        ...
        os.system(
            f"""powershell -command "Get-AppxPackage -allusers {uwp} """
...
CMDTweak(
    "Optimizing script after login",
    "Runs a script that lowers the priority of unnecessary processes",
    "StartupBat",
    recommended=True,
)
CMDTweak(
...
CMDTweak(
    "Stop Search Indexer", "Disables file indexing, useful for HDD", "StopSearchIndexer"
)
